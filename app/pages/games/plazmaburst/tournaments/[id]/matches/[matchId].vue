...
// 2. 动态注入 Head 和 Open Graph 标签
if (match.value) {
  // 消除边界情况：确保即便某些字段为空，也能优雅降级
  const title = `${match.value.redTeam.name || 'Unknown'} vs ${match.value.blueTeam.name || 'Unknown'} - ${match.value.phase || '?'}`
  const description = `Status: ${match.value.status || 'TBA'} | ${match.value.tournamentTitle}`
  ...
  useSeoMeta({
    ...
    // ogImage: 'https://your-domain.com/default-match-banner.jpg' // 如果你有动态生成的封面图，放这里
  })
} else {
  // 找不到比赛时的后备方案，Never break userspace
  useSeoMeta({
    title: 'Match Not Found - ForestWork',
  })
}
...
const isAbnormalMatch = computed(() => {
  return ['Walkover_Blue', 'Walkover_Red', 'Cancelled'].includes(match.value?.status)
})

const abnormalMatchMessage = computed(() => {
  switch (match.value?.status) {
    case 'Walkover_Blue':
      return `${match.value.redTeam.name} abstain the match, therefore ${match.value.blueTeam.name} wins by Walkover.`
    case 'Walkover_Red':
      return `${match.value.blueTeam.name} abstain the match, therefore ${match.value.redTeam.name} wins by Walkover.`
    case 'Cancelled':
      return 'The Match is Cancelled as both team are unable to compete.'
    default:
      return ''
  }
})
...
<template>
  ...
  <div v-else-if="match" class="max-w-6xl mx-auto p-4 md:p-8 space-y-6 animate-fade-in">
    ...
    <div v-if="isAbnormalMatch" class="text-center bg-gray-50 dark:bg-[#1a1c23] border border-red-900/50 p-6 md:p-8 rounded-xl shadow-sm space-y-10">
      <UIcon name="i-lucide-shield-alert" class="w-16 h-16 text-red-500 mb-4 mx-auto opacity-80" />
      <h2 class="text-2xl font-bold dark:text-white mb-2 uppercase tracking-wider">
        Match Terminated
      </h2>
      <p class="text-lg text-gray-300 font-medium">
        {{ abnormalMatchMessage }}
      </p>
    </div>
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-4 flex-1 justify-end">
            <div
              class="font-black text-xl md:text-3xl tracking-tight text-gray-900 dark:text-white truncate text-right">
              {{ match.blueTeam.name }}
            </div>
              ...
              <div class="flex gap-1.5">
                <div v-for="roundNum in totalRounds" :key="roundNum" class="flex flex-col gap-1 w-8 items-center">
                  ...
                  <div class="w-full h-8 flex items-center justify-center rounded-sm transition-colors shadow-sm"
                    ...
                    <UIcon v-if="displayData.roundHistory[roundNum - 1] === 1" name="i-lucide-crosshair" class="w-4 h-4"
                      :class="getContrastClass(match.redTeam.colour)" />
                  </div>
                  ...
                  <div class="w-full h-8 flex items-center justify-center rounded-sm transition-colors shadow-sm"
                    ...
                    <UIcon v-if="displayData.roundHistory[roundNum - 1] === 2" name="i-lucide-crosshair" class="w-4 h-4"
                      :class="getContrastClass(match.blueTeam.colour)" />
                  </div>
                </div>
              </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-8 border-t border-gray-200 dark:border-gray-800 mt-8">
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-2 mb-4">
              ...
              <h4 class="font-black text-gray-900 dark:text-white uppercase tracking-wider text-sm">{{
                ...
                <tbody>
                  <tr v-for="player in displayData.redTeamStats" :key="player.id"
                    ...
                    <td class="py-2.5 px-3 font-mono font-bold"
                      :class="Number(player.kdr) >= 1 ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'">
                      {{ player.kdr }}</td>
                    <td class="py-2.5 px-3 font-mono text-gray-500">{{ player.headshots }}</td>
                    <td class="py-2.5 pl-3 font-mono font-bold"
                      :class="player.aces > 0 ? 'text-amber-500' : 'text-gray-300 dark:text-gray-700'">{{ player.aces ||
                      '-' }}</td>
                  </tr>
                </tbody>
              </table>
              <!-- 無數據防呆 -->
              <div v-if="!displayData.redTeamStats || displayData.redTeamStats.length === 0"
                class="py-8 text-center text-xs text-gray-400 italic">No player stats available</div>
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div class="overflow-x-auto [&::-webkit-scrollbar]:hidden">
              ...
              <!-- 無數據防呆 -->
              <div v-if="!displayData.blueTeamStats || displayData.blueTeamStats.length === 0"
                class="py-8 text-center text-xs text-gray-400 italic">No player stats available</div>
            </div>
          </div>
