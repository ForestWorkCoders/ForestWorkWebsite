...
<template>
  ...
  <div v-else class="min-h-screen bg-gray-50 dark:bg-[#1a1b26] bg-[url('https://webusstatic.yo-star.com/mj-us-tournament-h5/prod/assets/bg.e1efdef8.png')] bg-cover bg-fixed bg-center">
    
    <div class="min-h-screen bg-black/50 backdrop-blur-sm pt-20 pb-12">
      
      <UContainer class="max-w-6xl">
        <div class="bg-white/90 dark:bg-[#1a1b26] text-gray-900 dark:text-white rounded-t-xl overflow-hidden shadow-2xl flex flex-col md:flex-row p-8 md:p-16 gap-12 items-center md:items-start border border-gray-200 dark:border-gray-800 transition-colors duration-300">
          ...
          <div class="flex-1 w-full space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-8 gap-x-6">
              ...
              <div class="space-y-1">
                ...
                <p class="text-sm font-semibold" :class="tourney.status.includes('進行中') ? 'text-green-400' : 'text-gray'">
                  {{ tourney.status }}
                </p>
              </div>
        ...
        <div class="bg-[#1e293b] w-full border-b border-gray-700">
          <UTabs 
            :items="tabs" 
            class="w-full"
            :ui="{
          ...
          >
            <template #info>
              <div class="bg-gray-200 dark:bg-[#cbd5e1] w-full h-[600px] mt-8 rounded-lg flex items-center justify-center">
                <span class="text-gray-500 font-bold tracking-widest">CONTENT PLACEHOLDER</span>
              </div>
            </template>
            
            <template #result><div class="h-[600px] mt-8 bg-gray-200 dark:bg-[#cbd5e1] rounded-lg"></div></template>
            <template #stats><div class="h-[600px] mt-8 bg-gray-200 dark:bg-[#cbd5e1] rounded-lg"></div></template>
            <template #vods><div class="h-[600px] mt-8 bg-gray-200 dark:bg-[#cbd5e1] rounded-lg"></div></template>
